...
export class UserCommand extends Command {
	...
	public async messageRun(message: GuildMessage) {
		...
		if (!channel) return reply(message, 'commands/music:joinNoVoicechannel');
	...
	private resolvePermissions(message: GuildMessage, voiceChannel: VoiceBasedChannelTypes): void {
		...
		if (voiceChannel.full && !permissions.has(Permissions.FLAGS.ADMINISTRATOR)) {
			...
			reply(message, ':octagonal_sign: Uh oh, the voice channel is currently full!');
		}
		if (!permissions.has(Permissions.FLAGS.CONNECT)) {
			// this.error(';x: Permissions Error: I am missing permission to join this channel!');
			reply(message, ';x: Permissions Error: I am missing permission to join this channel!');
		}
		if (!permissions.has(Permissions.FLAGS.SPEAK)) {
			// this.error(':octagonal_sign: Permissions Error: I am missing permission to speak in this channel!');
			reply(message, ':octagonal_sign: Permissions Error: I am missing permission to speak in this channel!');
		}
	}
